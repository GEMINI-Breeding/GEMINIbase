...
The trait/sensor/dataset/procedure/script/model record tables carry the
human-readable names of their parent entities alongside the FK ids for fast
filtering and streaming. A rename on a referenced entity has to be fanned
out to every affected record table — otherwise the record-lookup paths
(which search by name) silently return empty results after a rename.
...
Genotype calls are no longer one-row-per-call in Postgres (Phase 9d'); they
live in PGEN files in MinIO with a metadata-only catalog. Renames of
variants/accessions/studies are handled by the catalog tables' joined
views, so they don't need a cascade entry here.
"""
