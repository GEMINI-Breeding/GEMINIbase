"""Tests for the Script controller."""
import pytest
from unittest.mock import patch, MagicMock
from gemini.rest_api.models import (
    ScriptOutput, ExperimentOutput, ScriptRunOutput,
    DatasetOutput, ScriptRecordOutput
)


API_PATH = "gemini.rest_api.controllers.script.Script"
RECORD_PATH = "gemini.rest_api.controllers.script.ScriptRecord"
...
class TestGetAllScripts:
    ...
    def test_success(self, mock_cls, test_client, mock_output):
        mock_cls.get_all.return_value = [ScriptOutput(**mock_output)]
        response = test_client.get("/api/scripts/all")
...
class TestGetScripts:
    ...
    def test_search_success(self, mock_cls, test_client, mock_output):
        mock_cls.search.return_value = [ScriptOutput(**mock_output)]
        response = test_client.get("/api/scripts", params={"script_name": "Script A"})
        assert response.status_code == 200

    @patch(API_PATH)
    def test_search_not_found(self, mock_cls, test_client):
        mock_cls.search.return_value = None
        response = test_client.get("/api/scripts", params={"script_name": "Missing"})
        assert response.status_code == 200
        assert response.json() == []
...
class TestGetScriptById:
    ...
    @patch(API_PATH)
    def test_success(self, mock_cls, test_client, mock_output):
        mock_cls.get_by_id.return_value = ScriptOutput(**mock_output)
        response = test_client.get("/api/scripts/id/script-uuid")
        assert response.status_code == 200

    @patch(API_PATH)
    def test_not_found(self, mock_cls, test_client):
        mock_cls.get_by_id.return_value = None
        response = test_client.get("/api/scripts/id/missing")
...
class TestCreateScript:
    ...
    @patch(API_PATH)
    def test_success(self, mock_cls, test_client, mock_output):
        mock_cls.create.return_value = ScriptOutput(**mock_output)
        response = test_client.post("/api/scripts", json={
            "script_name": "Script A",
        })
...
class TestUpdateScript:
    ...
    def test_success(self, mock_cls, test_client, mock_output):
        mock_obj = MagicMock()
        mock_obj.update.return_value = ScriptOutput(**mock_output)
        mock_cls.get_by_id.return_value = mock_obj
        response = test_client.patch("/api/scripts/id/script-uuid", json={
            "script_name": "Updated",
        })
        assert response.status_code == 200
    ...
    def test_not_found(self, mock_cls, test_client):
        mock_cls.get_by_id.return_value = None
        response = test_client.patch("/api/scripts/id/missing", json={
            "script_name": "Updated",
        })
        assert response.status_code == 404
...
class TestDeleteScript:
    ...
    def test_success(self, mock_cls, test_client):
        ...
        mock_cls.get_by_id.return_value = mock_obj
        response = test_client.delete("/api/scripts/id/script-uuid")
        assert response.status_code in (200, 204)

    @patch(API_PATH)
    def test_not_found(self, mock_cls, test_client):
        mock_cls.get_by_id.return_value = None
        response = test_client.delete("/api/scripts/id/missing")
        assert response.status_code == 404
...
class TestScriptAssociations:
    ...
    def test_get_experiments(self, mock_cls, test_client):
        ...
        mock_cls.get_by_id.return_value = mock_obj
        response = test_client.get("/api/scripts/id/script-uuid/experiments")
        assert response.status_code == 200

    @patch(API_PATH)
    def test_get_runs(self, mock_cls, test_client):
        mock_obj = MagicMock()
        mock_obj.get_associated_runs.return_value = [ScriptRunOutput(
            id="r1", script_name="Script A", script_run_info={}
        )]
        mock_cls.get_by_id.return_value = mock_obj
        response = test_client.get("/api/scripts/id/script-uuid/runs")
        assert response.status_code == 200
    ...
    def test_get_datasets(self, mock_cls, test_client):
        ...
        mock_cls.get_by_id.return_value = mock_obj
        response = test_client.get("/api/scripts/id/script-uuid/datasets")
        assert response.status_code == 200

    @patch(API_PATH)
    def test_association_not_found(self, mock_cls, test_client):
        mock_cls.get_by_id.return_value = None
        response = test_client.get("/api/scripts/id/missing/experiments")
        assert response.status_code == 404
    ...
    def test_get_runs_not_found(self, mock_cls, test_client):
        mock_cls.get_by_id.return_value = None
        response = test_client.get("/api/scripts/id/missing/runs")
        assert response.status_code == 404
    ...
    def test_get_datasets_not_found(self, mock_cls, test_client):
        mock_cls.get_by_id.return_value = None
        response = test_client.get("/api/scripts/id/missing/datasets")
        assert response.status_code == 404
...
class TestScriptRecords:
    ...
    def test_get_record_by_id_success(self, mock_record_cls, test_client):
        mock_record_cls.get_by_id.return_value = ScriptRecordOutput(
            id="r1", timestamp="2024-01-01T00:00:00", script_id="s1",
            script_name="Script A", script_data={}, dataset_id="d1",
    ...
    def test_delete_record_success(self, mock_record_cls, test_client):
        ...
        response = test_client.delete("/api/scripts/records/id/r1")
        assert response.status_code in (200, 204)

    @patch(RECORD_PATH)
    def test_delete_record_not_found(self, mock_record_cls, test_client):
        mock_record_cls.get_by_id.return_value = None
        response = test_client.delete("/api/scripts/records/id/missing")
        assert response.status_code == 404
    ...
    def test_update_record_success(self, mock_record_cls, test_client):
        mock_record = MagicMock()
        mock_record.update.return_value = ScriptRecordOutput(
            id="r1", timestamp="2024-01-01T00:00:00", script_id="s1",
            script_name="Script A", script_data={"updated": True}, dataset_id="d1",
            dataset_name="DS", collection_date=None,
            experiment_id=None, experiment_name=None,
            season_id=None, season_name=None,
            site_id=None, site_name=None,
        ...
        mock_record_cls.get_by_id.return_value = mock_record
        response = test_client.patch("/api/scripts/records/id/r1", json={
            "script_data": {"updated": True},
        })
        assert response.status_code == 200
    ...
    def test_update_record_not_found(self, mock_record_cls, test_client):
        mock_record_cls.get_by_id.return_value = None
        response = test_client.patch("/api/scripts/records/id/missing", json={
            "script_data": {},
        })
        assert response.status_code == 404
